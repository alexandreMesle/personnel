...
public class EmployeConsole {
	private Option afficher(final Employe employe) {
		return new Option("Afficher l'employé", "l", () -> {
			System.out.println("Date d'arrivée :" + employe.getDateA());
			System.out.println("Date de départ :" + employe.getDateD());
		});
	}
...
//			menu.add(changerDateDepart(employe));
	...
	private Option changerDateDepart(final Employe employe) {
		...
		return new Option("Changer la date de départ", "d", () -> {
			try {
				employe.setDateA(LocalDate.parse(getString("Date de départ (AAAA-MM-JJ) : ")));
